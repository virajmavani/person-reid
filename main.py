...
with tf.Session() as sess:
    ...
    while cap.isOpened():
        ...
        if pick_previous is not None:
            for idx1, (x1, y1, x2, y2) in enumerate(pick_previous):
                print(pick_previous[idx1])
                image_1 = image_previous[y1:y2, x1:x2]
                for idx2, (xA, yA, xB, yB) in enumerate(pick):
                    image_2 = image[yA:yB, xA:xB]
                    image1 = cv2.resize(image_1, (IMAGE_WIDTH, IMAGE_HEIGHT))
                    image1 = cv2.cvtColor(image1, cv2.COLOR_BGR2RGB)
                    image1 = np.reshape(image1, (1, IMAGE_HEIGHT, IMAGE_WIDTH, 3)).astype(float)

                    image2 = cv2.resize(image_2, (IMAGE_WIDTH, IMAGE_HEIGHT))
                    ...
                    if bool(not np.argmax(prediction[0])):
                        cv2.putText(image, str(idx1+1), (xA, yA), font, 1, (0, 0, 0), 2, cv2.LINE_AA)
                        if idx1 != idx2:
                            if idx1 > idx2:
                                # print(pick)
                                temp = np.zeros(pick_previous.shape, np.uint8)
                                temp[:pick.shape[0], :pick.shape[1]] = pick
                                pick = temp
                                print("----------")
                                print(pick)
                                print("----------")
                            pick[idx1], pick[idx2] = pick[idx2], pick[idx1]
